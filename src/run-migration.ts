...
const insertMigration = async (
  migrationTableName: string = 'migrations',
  migrationSchemaName: string = 'public',
  client: BasicPgClient,
  migration: Migration,
  log: Logger,
) => {
  log(
    `Saving migration to '${migrationSchemaName}.${migrationTableName}': ${migration.id} | ${migration.name} | ${migration.hash}`,
  )
